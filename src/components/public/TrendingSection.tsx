      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 stagger-children">
        {articles.map((article, i) => (
          <Link
            ...
            className="group relative flex sm:flex-col items-start gap-3 sm:gap-0 bg-surface-card rounded-2xl border border-surface-border overflow-hidden card-hover"
          >
            {/* Image */}
            <div className="relative w-20 h-20 sm:w-full sm:h-0 sm:pb-[60%] flex-shrink-0 overflow-hidden">
              ...
              {/* Rank number */}
              <span className="absolute top-1.5 left-1.5 sm:top-2 sm:left-2 w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center bg-heading text-white font-extrabold text-sm sm:text-base rounded-lg shadow-lg">
                {i + 1}
              </span>
            </div>
